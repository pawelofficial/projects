...
#            base_col_vals=','.join([f"{row_d[x]}" for x in df_base_cols])
            ...
            log_stuff(msg=f'executed query {INSERT}')
...
# recreates  necessary objects for the bot - manually recreate the schema to clean things up 
def recreate_objects():
    p=mydb()
    p.schema='public'
    p.connect()
    p.cur = p.conn.cursor()
    p.cur.execute(f"SET search_path TO public")
    p.execute_dml('drop schema dev cascade')
    p=mydb()
    p.execute_dml('create schema dev')
